                            ...
                            @if($ultimo)
                                <div class="text-sm space-y-2">
                                    <p class="flex items-center text-gray-600 dark:text-gray-300">
                                        <svg class="h-4 w-4 mr-2 flex-shrink-0 text-indigo-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" />
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z" />
                                        </svg>
                                        <span class="truncate">{{ $ultimo->ubicacion ? $ultimo->ubicacion->nombre : 'Sin ubicación' }}</span>
                                    </p>
                                    @if($ultimo->referido)
                                    <p class="flex items-center text-gray-600 dark:text-gray-300">
                                        <svg class="h-4 w-4 mr-2 flex-shrink-0 text-indigo-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        ...
                                        <span class="truncate">{{ $ultimo->referido }}</span>
                                    </p>
                                    @endif
                                    @if($persona->puesto_votacion || $persona->mesa_votacion)
                                    <p class="flex items-start text-gray-600 dark:text-gray-300 text-xs">
                                        <svg class="h-4 w-4 mr-2 flex-shrink-0 text-purple-500 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4" />
                                        </svg>
                                        <span>
                                            Vota: {{ $persona->puesto_votacion ?? 'N/A' }}
                                            @if($persona->mesa_votacion) - Mesa #{{ $persona->mesa_votacion }}@endif
                                        </span>
                                    </p>
                                    @endif
                                    <p class="mt-4 text-xs text-gray-400 text-right">
                                        {{ $ultimo->created_at->diffForHumans() }} por {{ $ultimo->user->name }}
                                    </p>
                                </div>
                            @else
